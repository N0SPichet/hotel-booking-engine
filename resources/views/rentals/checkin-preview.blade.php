	...
	<div class="row m-t-10">
		<div class="col-md-12">
			<div class="margin-content">
				...
				<p><b>Renter Short Details</b> @if (isset($rentals)){{$rentals[0]->user->verification->title}} {{$rentals[0]->user->verification->name}} {{$rentals[0]->user->verification->lastname}}, {{$rentals[0]->user->user_gender=='1'?'Male':'Female'}}@elseif (isset($rental)){{$rental->user->verification->title}} {{$rental->user->verification->name}} {{$rental->user->verification->lastname}}, {{$rental->user->user_gender=='1'?'Male':'Female'}}@endif</p>
				<div class="m-t-10">
				@if ($checkinBy == 'renter')
					@if (isset($rentals))
					{!! Form::open(['route'=>'rentals.checkin.confirmed', 'data-parsley-validate' => '']) !!}
						...
						<label class="">
							...
							<div class="stay-date"><i class="far fa-calendar-alt"></i> Stay Date : {{ date('jS F, Y', strtotime($rental->rental_datein)) }} <i class="fas fa-long-arrow-alt-right"></i> {{ date('jS F, Y', strtotime($rental->rental_dateout)) }} ({{ Carbon::parse($rental->rental_datein)->diffInDays(Carbon::parse($rental->rental_dateout)) }} {{ Carbon::parse($rental->rental_datein)->diffInDays(Carbon::parse($rental->rental_dateout))>'1'?'days':'day' }})</div>
